$(function () {
  ...
  var LockButton = function (btnObjId, remainTime) {
    //1.获取当前系统时间
    ...
    if (countdownTime === null || countdownTime === undefined ||
      countdownTime === 'undefined' || countdownTime === 'null') {
      var now = new Date().getTime();  //当前时间戳
      var endTime = remainTime * 1000 + now;  //结束时间戳
      $.cookie("countdownTime", endTime);  //将结束时间保存到cookie
    }
    $(btnObjId).attr('disabled', 'disabled')
      .val('(' + remainTime + ')秒后重新获取');
  ...
  //读取cookie
  if ($.cookie("countdownTime") !== undefined &&
    !isNaN($.cookie("countdownTime"))) {  //读取到了cookie值
